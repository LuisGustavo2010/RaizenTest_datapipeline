...
def download_file():
    try:
        url = 'https://github.com/LuisGustavo2010/Datapipeline_Test_Raizen/raw/main/data/raw_data/vendas-combustiveis-m3.xls'
        filename = 'vendas-combustiveis-m3.xls'

        response = requests.get(url, stream=True)

        with open(os.path.join(r"data\raw_data", filename), "wb") as f:
...
def Pandas_transform():
    #Concatenando dfs
    try:
      file = r'data\raw_data\vendas-combustiveis-m3.xls'
    ...
    #tratamentos
    try: 
      path= r'data\silver\cleanData.csv'
...
def VendasDerivadosPetroleo():
    path = r'data\gold\VendasDerivadosPetroleo.csv'
    df = pd.read_csv(r'data\silver\cleanData.csv', sep=';')
...
def VendasDiesel():
    path= r'data\gold\VendasDiesel.csv'
    df = pd.read_csv(r'data\silver\cleanData.csv', sep=';')
