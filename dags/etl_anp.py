...
def download_file():
    try:
        ...
        with open(os.path.join(r"C:\Users\luisg\OneDrive\Luis Gustavo\Documentos\Projetos\Datapipeline_test_raizen\data\raw_data", filename), "wb") as f:
    ...
    try:
        subprocess.run(['libreoffice', '--headless', '--convert-to', 'ods', '--outdir', r'data\raw_data', 'vendas-combustiveis-m3.xls'])
    except:
        pass
...
def Pandas_transform():
    #Concatenando dfs
    try:
      file = r'C:\Users\luisg\OneDrive\Luis Gustavo\Documentos\Projetos\Datapipeline_test_raizen\data\raw_data\vendas-combustiveis-m3.xls'
    ...
    #tratamentos
    try: 
      path= r'C:\Users\luisg\OneDrive\Luis Gustavo\Documentos\Projetos\Datapipeline_test_raizen\data\silver\cleanData.csv'
...
def VendasDerivadosPetroleo():
    path = r'C:\Users\luisg\OneDrive\Luis Gustavo\Documentos\Projetos\Datapipeline_test_raizen\data\gold\VendasDerivadosPetroleo.csv'
    df = pd.read_csv(r'C:\Users\luisg\OneDrive\Luis Gustavo\Documentos\Projetos\Datapipeline_test_raizen\data\silver\cleanData.csv', sep=';')
...
def VendasDiesel():
    path= r'C:\Users\luisg\OneDrive\Luis Gustavo\Documentos\Projetos\Datapipeline_test_raizen\data\gold\VendasDiesel.csv'
    df = pd.read_csv(r'C:\Users\luisg\OneDrive\Luis Gustavo\Documentos\Projetos\Datapipeline_test_raizen\data\silver\cleanData.csv', sep=';')
    df = df[df['product'] != 'ÓLEO DIESEL']
    df = df[df['product'].str.startswith('ÓLEO DIESEL')]
    df.to_csv(path, index=False, sep=';')

with DAG('luis_datapipeline', 
  ...
  vendas_derivados_petroleo = PythonOperator(
      task_id = 'vendas_derivados_petroleo',
      python_callable = VendasDerivadosPetroleo
  )
  VendasDiesel = PythonOperator(
      task_id = 'vendas_diesel',
      python_callable = VendasDiesel
  )

download_file >> pandas_transform >> vendas_derivados_petroleo >> VendasDiesel